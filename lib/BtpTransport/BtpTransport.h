...
/**
 * @brief Rebinds the shared (source_id, boot_id, sequence) counter this
 * module reads/writes through to `endpoint` instead of this module's own
 * default-constructed one.
 *
 * BTP/docs/model.md section 4.3: "the sequence space is shared by all
 * message types generated by the same producer" -- not per transport. This
 * dongle speaks BTP over two physical links (ESP-NOW, USB serial), and the
 * serial side's btp::Node (BTP >= topico 37) owns the one Endpoint whose
 * session-responder auto-replies (HELLO_RESULT/SESSION_CLOSE_RESULT) cannot
 * be redirected to any other Endpoint -- so THAT Endpoint has to be the one
 * every other send on either transport reserves its sequence from too.
 * SerialMux::begin() calls this once, after its own btp::Node has begun (so
 * source_id/boot_id are already configured on it); every BtpTransport call
 * before that point (and every native test that never calls this at all)
 * keeps using this module's own private Endpoint, configured the normal way
 * through configureIdentity().
 */
void bindEndpoint(btp::Endpoint& endpoint) noexcept;
